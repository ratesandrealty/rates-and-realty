...
| `ocr-mms-upload` | A | shared secret compare, HEADER ONLY since 2026-08-15 (?secret= removed); secret is a committed literal — docs/OCR-SHARED-SECRET-2026-08-15.md � L112: if (secret !== SHARED_SECRET) return new Response('Forbidden', { status: 403 }) | service_role | no browser caller found | db trigger_ocr_on_uploaded_document; fn:sms-assistant |
