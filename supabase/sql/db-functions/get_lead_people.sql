...
AS $function$
begin
  /* STAFF ONLY — the widest of the seven. SECURITY DEFINER bypasses RLS and
     every mask, the arguments are just a contact id, and the result carries
     email, phone, secondary_phone, DATE OF BIRTH and SSN_LAST4 for that
     borrower and everyone connected to them. Any signed-in user could name any
     uuid; anon held EXECUTE, so the public anon key was enough.
     LANGUAGE changed from sql to plpgsql only so the guard can run before the
     query — the query below is unchanged. */
    ...
    select
      coalesce(
        ...
        (select id from mortgage_applications
          where mortgage_applications.contact_id = p_contact_id
