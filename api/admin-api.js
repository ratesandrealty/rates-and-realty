...
export async function getAdminDashboardData() {
  const [contactsResult, leadsResult, applicationsResult, documentsResult, notesResult, tasksResult] = await Promise.all([
    ...
    supabase.from("mortgage_applications").select("*, contacts(id, first_name, last_name, email, phone, credit_score, monthly_income, pipeline_status, score_tier, loan_type)").order("updated_at", { ascending: false }),
