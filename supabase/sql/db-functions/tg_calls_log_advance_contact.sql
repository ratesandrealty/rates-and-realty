...
AS $function$
declare
  v_out   text := lower(coalesce(new.outcome,''));
  v_title text;
  v_ae_id uuid;
  v_first_contact boolean;
begin
  v_first_contact :=
    (TG_OP = 'INSERT' and new.contact_id is not null)
    or (TG_OP = 'UPDATE' and old.contact_id is null and new.contact_id is not null);

  if v_first_contact then
    update contacts set
  ...
  select id into v_ae_id from activity_events
   where metadata->>'calls_log_id' = new.id::text limit 1;
  ...
  if v_ae_id is null then
    ...
    values (new.contact_id, new.contact_id, 'call', v_title,
            ...
            jsonb_build_object('outcome', new.outcome, 'duration', new.duration,
                               ...
                               'calls_log_id', new.id))
    on conflict ((metadata->>'calls_log_id')) where metadata->>'calls_log_id' is not null do nothing;
  else
    update activity_events set
      contact_id  = new.contact_id,
      lead_id     = new.contact_id,
      title       = v_title,
      description = nullif(trim(coalesce(new.notes,'')),''),
      direction   = new.direction,
      status      = new.status,
      metadata    = jsonb_build_object('outcome', new.outcome, 'duration', new.duration,
                                       'recording_url', new.recording_url, 'twilio_call_sid', new.twilio_call_sid,
                                       'calls_log_id', new.id)
    where id = v_ae_id;
  end if;
