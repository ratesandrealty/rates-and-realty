...
CREATE OR REPLACE FUNCTION public.loan_orders_for_contact(p_contact_id uuid)
 RETURNS TABLE(id uuid, order_type text, status text, label text, vendor_id uuid, vendor_name text, vendor_company text, borrower_contact_id uuid, borrower_name text, employer_name text, hr_contact_name text, hr_contact_first_name text, hr_contact_last_name text, hr_contact_phone text, hr_contact_email text, reference text, notes text, ordered_at timestamp with time zone, received_at timestamp with time zone, follow_up_at timestamp with time zone, follow_up_owner text, follow_up_task_id uuid, last_follow_up_at timestamp with time zone, last_note_at timestamp with time zone, note_count integer)
 LANGUAGE plpgsql
