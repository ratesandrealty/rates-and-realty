...
AS $function$
  with rows_all as (
    ...
    select a.*,
           ...
           /* WHICH message, so the board can deep-link to the reply itself rather
              than to the top of the thread. Ordered by received_at to pair with
              last_reply_at above -- max(received_at) and "the id of the newest"
              have to be the SAME row or the card opens at the wrong message. */
           (select q.gmail_message_id from quote_reply_log q
             where q.kind = a.family and q.row_id = a.id
               and q.gmail_message_id is not null
             order by q.received_at desc limit 1) as last_reply_msg_id
