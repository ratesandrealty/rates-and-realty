...
| ~~`bot-admin`~~ | **CLOSED 2026-08-19 — `requireStaff(req, { roles: ['admin'] })`.** Admin-only, not the default STAFF_ROLES: its one page (`dashboard/admin.html`) gates on a hardcoded `ADMIN_EMAILS` allowlist of one address, so staff-wide data behind an admin-only page would repeat the `insights-data` mistake. Frontend-first — `botAdmin()` sent `_SB_HEADERS`, the anon key hardcoded in that page, and moved to `fnFetch` first. **Residual: the page itself is unconfirmed by automation** (that allowlist redirects the test account); a human must open the AI Agent tab |
| ~~`lead-scorer`~~ | **CLOSED 2026-08-19 — `requireStaff(req)`.** STAFF_ROLES, deliberately *unlike* `bot-admin`: its callers are on `admin/lead-detail.html`, a shared staff page, so admin-only would take a working surface from the VA. Four internal callers all send the service key. Frontend-first: both anon-key call sites moved to `fnFetch` and confirmed with a live session by a spec that drives the page's own `scorerApi` |
