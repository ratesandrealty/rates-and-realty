...
by what each one can actually **do** â€” because the order they get worked in
...
but those are different sets measured by different criteria â€” the doc's 19 are
...
- `verify_jwt` in `config.toml` â€” the anon key satisfies it and is public
...
the same mistake â€” matching text that mentions a control rather than performs
one:

| function | what matched | why it is not authorization |
|---|---|---|
| `proactive-followups` | `// Authorized via x-cron-secret headerâ€¦` | a comment |
| `gdrive-health-monitor` | `.from("auth_user_roles")` | inside `notifyInCrm()` â€” finds admins to NOTIFY |
| `loan-date-nudges` | `sms_authorized_phones` | it is `summary.recipients` â€” who to text, not who may call |
...
| | meaning |
|---|---|
| **A** | Sends messages â€” SMS, email, voice. Leaves the system under the business identity |
| **B** | Writes borrower data â€” database, Drive or storage mutations |
| **C** | Spends money or AI credits â€” a paid third-party API on every call |
| **D** | Read-only â€” no send, write or paid call found |
...
| `voe-inbound-poll` | D | shared secret compare — L67: if (s !== POLL_SECRET) return new Response(JSON.stringify({ ok: false, error: 'unauthorized' }), { status: 401 | service_role + user JWT | no browser caller found | cron voe-inbound-poll-10min [*/10 * * * *] |
